...
# Run: .\download_pass_models.ps1
# From: c:\Users\adsfo\Documents\scripts\job_app_tailor

$root = $PSScriptRoot
$py = Join-Path $root "venv\Scripts\python.exe"
if (-not (Test-Path $py)) {
  throw "Python venv not found at: $py (create venv + install requirements first)"
}
...
def existing_target_path(repo_id: str, filename: str) -> Path:
    # Mirror HubManager's internal cache location.
    return Path(hub._cache_dir) / _safe_dirname(repo_id) / filename


def reconcile_root_file(repo_id: str, filename: str) -> Path:
    """
    If a GGUF is sitting directly under HUB_CACHE_DIR, move it into the
    expected subfolder layout so we don't download duplicates.
    """
